...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        db.create_table('core_actionparameter', (
        ...
        db.send_create_signal('core', ['ActionParameter'])

        # Adding unique constraint on 'ActionParameter', fields ['action', 'name']
        db.create_unique('core_actionparameter', ['action_id', 'name'])

        # Adding unique constraint on 'ActionParameter', fields ['action', 'parameter_position']
        db.create_unique('core_actionparameter', ['action_id', 'parameter_position'])


    def backwards(self, orm):
        # Removing unique constraint on 'ActionParameter', fields ['action', 'parameter_position']
        db.delete_unique('core_actionparameter', ['action_id', 'parameter_position'])

        # Removing unique constraint on 'ActionParameter', fields ['action', 'name']
        db.delete_unique('core_actionparameter', ['action_id', 'name'])

        # Deleting model 'ActionParameter'
        db.delete_table('core_actionparameter')
    ...
    models = {
        ...
        'core.actionparameter': {
            'Meta': {'unique_together': "(('action', 'name'), ('action', 'parameter_position'))", 'object_name': 'ActionParameter'},
            'action': ('django.db.models.fields.related.ForeignKey', [], {'to': "orm['core.Action']"}),
            'created_at': ('django.db.models.fields.DateTimeField', [], {'auto_now_add': 'True', 'blank': 'True'}),
            'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            'name': ('django.db.models.fields.CharField', [], {'max_length': '255'}),
