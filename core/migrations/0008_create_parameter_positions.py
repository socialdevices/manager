...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for action in orm.Action.objects.all():
            i = 0
            for actiondevice in orm.ActionDevice.objects.filter(action=action).order_by('created_at'):
                actiondevice.parameter_position = i
                actiondevice.save()
                i += 1


    def backwards(self, orm):
        for actiondevice in orm.ActionDevice.objects.all():
            actiondevice.parameter_position = None
            actiondevice.save()
